...
parser.add_argument(
    "--max_fcst", type=int, help="max_fcst", required=False, default=np.inf
)
parser.add_argument("--gp_error", type=str2bool, help="", required=False, default=False)
...
def run(config, max_fcst=np.inf, gp_error=True):
    ...
    model.load_state_dict(
        torch.load("models/" + config["train"]["fname"], weights_only=True)
    )
