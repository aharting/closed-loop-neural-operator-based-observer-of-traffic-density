...
parser.add_argument(
    "--config", type=str, help="Base operator config file path", required=True
)
parser.add_argument(
    "--max_unroll", type=int, help="max unroll", required=False, default=np.inf
)
...
def run(config, max_unroll=np.inf):
    ...
    model.load_state_dict(
        torch.load(
            "models/" + config["train"]["fname"], weights_only=True, map_location=device
        )
    )
    model.eval()
