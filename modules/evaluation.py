...
def inspect_observers(model, loader, config, device, deltaX, deltaT, T_in, T_out, id='test', max_fcst=np.inf, gp_error=True, std_y=0):
    ...
    for i in range(y_cl.shape[0]):
        ...
        plot_inspection(frame_pred=y_cl[i],
                            frame_true=y_true[i],
...
def report_accuracy(model, loaders, config, device, deltaX, deltaT, T_in, T_out, id='test', max_fcst=np.inf):
    ...
    for std_y in std_ys:
        ...
        scores[fr"$\sigma={std_y}$"]["Closed loop"] = frame_scores_cl
        scores[fr"$\sigma={std_y}$"]["Open loop with reset"] = frame_scores_olr
        scores[fr"$\sigma={std_y}$"]["Open loop"] = frame_scores_ol
        ...
        if std_y == 0:
            ...
            plot_l2_error_evolution_unformatted(scores_cl, scores_olr, scores_ol, T_in, T_out, deltaT, fname=f"{dir}/{id}_batch_l2_error_distribution")
            plot_l2_error_evolution_formatted(scores_cl, scores_olr, scores_ol, T_in, T_out, deltaT, fname=f"{dir}/{id}_batch_l2_error_distribution_formatted")
            ...
            np.save(f"{dir}/unroll_{max_fcst}_{id}_frame_scores_cl.npy", frame_scores_cl)
            np.save(f"{dir}/unroll_{max_fcst}_{id}_frame_scores_ol.npy", frame_scores_ol)
            np.save(f"{dir}/unroll_{max_fcst}_{id}_frame_scores_olr.npy", frame_scores_olr)
...
def report_unrolled(model, loader, config, device, deltaX, deltaT, T_in, T_out, id='test', max_fcst=np.inf):
    ...
    for i in range(y_cl.shape[0]):
        ...
        plot_unrolled(frame_true=y_true[i],
                    frame_cl=y_cl[i],
                    frame_ol=y_ol[i],
                    frame_olr=y_olr[i],
